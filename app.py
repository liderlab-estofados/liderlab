    ...
    try:
        ...
        for row in rows:
            ...
            models_by_category[categoria].append(
                {"modelo": row["MODELO"], "qtd": row["qtd"], "categoria": categoria}
            )
...
def api_modules():
    ...
    try:
        ...
        modules = []
        for row in rows:
            raw_img = row["image"] or ""
            # use only the basename so we never double‑prefix
            filename = os.path.basename(raw_img)
            if filename:
                img = url_for("static", filename=f"images/{filename}")
            else:
                img = ""
...
#         results = [{"modelo": row["MODELO"], "type": "model"} for row in rows]
...
#             return (
#                 jsonify(
#                     {"message": "Módulo cadastrado com sucesso!", "filename": filename}
#                 ),
#                 201,
#             )
